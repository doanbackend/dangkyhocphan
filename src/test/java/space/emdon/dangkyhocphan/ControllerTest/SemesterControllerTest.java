...
@MockBean SemesterService semesterService;
...
void createSemesterController_invalidRequest_success() throws Exception {
	...
	when(semesterService.createSemester(any())).thenReturn(semesterResponse);

	mockMvc
		.perform(
			MockMvcRequestBuilders.post("/semesters")
				.with(csrf())
		...
		.andExpect(MockMvcResultMatchers.jsonPath("$.result.startDate").value("2025-08-10"))
		.andExpect(MockMvcResultMatchers.jsonPath("$.result.endDate").value("2025-12-15"))
		.andExpect(
			MockMvcResultMatchers.jsonPath("$.result.registrationStartDate")
...
void createSemesterController_nameInvalid_fail() throws Exception {
	...
	mockMvc
		...
		.andExpect(MockMvcResultMatchers.jsonPath("$.code").value(1031));
}
	...
	mockMvc
		.perform(
			MockMvcRequestBuilders.post("/semesters")
				...
				.content(content))
		.andExpect(MockMvcResultMatchers.status().isBadRequest())
		.andExpect(MockMvcResultMatchers.jsonPath("$.code").value("1032"));
}
	...
	mockMvc
		.perform(
			MockMvcRequestBuilders.post("/semesters")
				...
				.content(content))
		.andExpect(MockMvcResultMatchers.status().isBadRequest())
		.andExpect(MockMvcResultMatchers.jsonPath("$.code").value("1033"));
}
	...
	mockMvc
		.perform(
			MockMvcRequestBuilders.post("/semesters")
				...
				.content(content))
		.andExpect(MockMvcResultMatchers.status().isBadRequest())
		.andExpect(MockMvcResultMatchers.jsonPath("$.code").value("1034"));
}
...
void updateSemesterController_invalidRequest_success() throws Exception {
	...
	mockMvc
		.perform(
			MockMvcRequestBuilders.put("/semesters/{name}", semesterName)
				.with(csrf())
				.contentType(MediaType.APPLICATION_JSON_VALUE)
				.content(content))
		.andExpect(MockMvcResultMatchers.status().isOk())
...
void getAllSemesterController_invalidRequest_success() throws Exception {
	...
	mockMvc
		.perform(
			MockMvcRequestBuilders.get("/semesters")
				.with(csrf())
...
void deleteSemesterController_invalidRequest_success() throws Exception {
	...
	mockMvc
		.perform(MockMvcRequestBuilders.delete("/semesters/{name}").with(csrf()))
		.andExpect(MockMvcResultMatchers.status().isOk());
	Mockito.verify(semesterService, Mockito.times(1)).deleteSemester(semesterName);
}
}
