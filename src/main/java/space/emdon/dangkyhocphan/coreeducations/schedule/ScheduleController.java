...
@GetMapping
ApiResponse<Page<ScheduleResponse>> getAllSchedules(@PageableDefault(page = 0, size = 20, sort = "dayOfWeek", direction = Sort.Direction.DESC)Pageable pageable) {
	return ApiResponse.<Page<ScheduleResponse>>builder()
		.result(scheduleService.getAllSchedules(pageable))
		.build();
}
