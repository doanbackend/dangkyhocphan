...
@Query(
	"SELECT COUNT(s) > 0 FROM Schedule s WHERE s.dayOfWeek = :dayOfWeek AND s.startPeriod = :startPeriod AND s.endPeriod = :endPeriod AND s.room = :room")
boolean existsByDayOfWeekAndStartPeriodAndEndPeriodAndRoom(
...
@Modifying
@Query(
	"UPDATE Schedule s SET s.startPeriod = :startPeriod, s.endPeriod = :endPeriod WHERE s.id = :id")
int updateSchedulePeriod(
	@Param("id") Long id,
	@Param("startPeriod") int startPeriod,
	@Param("endPeriod") int endPeriod);
